...
st.set_page_config(
    page_title="Bullet Inferno Roguelike",
    layout="centered",
...
const STORAGE_KEY = 'bulletInfernoRogueLeaderboardV1';
...
function boardHtml() {
    ...
    const rows = board.slice(0, 8).map((r,i) => `
        <tr>
            ...
            <td>${r.victory ? 'Victory' : 'Dead'}</td>
            <td>${r.bosses}/5</td>
            <td>${r.coins}</td>
            <td>${r.score}</td>
            <td>${formatTime(r.time)}</td>
        </tr>`).join('');
    return `<div class="leaderboard"><div class="leaderTitle">Leaderboard</div><table><thead><tr><th>#</th><th>Name</th><th>Run</th><th>Bosses</th><th>Coins</th><th>Score</th><th>Time</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function renderMenu() {
    ...
    const shipCards = Object.values(SHIPS).map(s => `
        <div class="shipCard ${selectedShipId === s.id ? 'selected' : ''}" data-ship="${s.id}">
    ...
    showOverlay(`
        <div class="card">
            ...
            <div class="smallNote">New in roguelike mode: random bosses, run relics, ship selection, revive with coins and a new run seed every game.</div>
        </div>
    `);
...
function completeBoss() {
    ...
    runStats.score += 1000 + boss.zone * 350;
...
function victory() {
    ...
    showOverlay(`
        <div class="card">
            ...
            <div class="subtitle">You broke the final boss without firing a single shot.</div>
            ${resultStatsHtml()}
            <div class="rowButtons">
...
function resultStatsHtml() {
    return `
        ...
        <div class="smallNote">Bombs collected: blue ${runStats.bombs.blue}, purple ${runStats.bombs.purple}, golden ${runStats.bombs.gold}. Seed: ${runStats.seed}.</div>
    `;
}
...
function damagePlayer() {
    ...
    playTone(150, 0.10, 'sawtooth', 0.05);
    if (player.lives <= 0) {
        showGameOver();
    }
}
...
function collectPickup(o) {
    if (o.type === 'coin') {
        ...
        playTone(860, 0.035, 'triangle', 0.028);
        ...
        hitBoss(mods.blueDamage, COLORS.blueBomb, 'blue');
    } else if (o.type === 'bomb_purple') {
        hitBoss(mods.purpleDamage, COLORS.purpleBomb, 'purple');
    } else if (o.type === 'bomb_gold') {
        hitBoss(mods.goldDamage, COLORS.goldBomb, 'gold');
    }
}

function hitBoss(dmg, color, kind) {
    if (!boss || boss.dead) return;
    boss.hp = Math.max(0, boss.hp - dmg);
    boss.hitTimer = 0.22;
    runStats.bombs[kind] += 1;
    runStats.score += Math.round(dmg * 50 + mods.bombScore * 100);
    addBurst(boss.x, boss.y, color, kind === 'gold' ? 34 : 18, kind === 'gold' ? 8 : 4, 1.0);
    addTextParticle(boss.x + (rand()*50-25), boss.y + boss.size + 8, `-${dmg}%`, color, 0.7, kind === 'gold' ? 26 : 18);
    playTone(kind === 'gold' ? 220 : 330, 0.07, 'square', 0.035);
...
function projectileKind() {
    ...
    if (r < pGold + pPurple + pBlue + pHeart) return 'heart';
    if (r < pGold + pPurple + pBlue + pHeart + pCoin) return 'coin';
    return 'bullet';
}
...
function spawnShot(x, y, angle, speed, radius=6, forceKind=null) {
    ...
    if (kind !== 'bullet') {
        sp *= 0.62;
        r = kind === 'bomb_gold' ? 13 : kind === 'bomb_purple' ? 11 : kind === 'bomb_blue' ? 10 : kind === 'heart' ? 10 : 8;
    }
    bullets.push({
        type: kind,
        x, y,
...
function drawUI() {
    ...
    ctx.fillText(` Bosses ${runStats.bossesKilled}/5`, x+505, y);
    ctx.textAlign = 'right';
...
function drawBullets() {
    for (const o of bullets) {
        ...
        if (o.type === 'bullet') {
            ...
            ctx.shadowBlur = o.type === 'bomb_gold' ? 22 : 14;
            ctx.shadowColor = color;
            ctx.fillStyle = color;
            ctx.beginPath(); ctx.arc(0,0,o.r,0,Math.PI*2); ctx.fill();
            ctx.fillStyle = 'rgba(255,255,255,0.88)';
            ctx.fillRect(-2,-o.r-6,4,7);
            ctx.strokeStyle = '#fff8ec'; ctx.lineWidth = 2;
...
function playTone(freq=440, duration=0.05, type='sine', volume=0.04) {
    try {
...
runStats = {zone:1,bossesKilled:0,coins:0,score:0,bombs:{blue:0,purple:0,gold:0},hearts:0,revives:0,seed:0,activeTime:0,victory:false};
