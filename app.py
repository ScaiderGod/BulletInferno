...
st.set_page_config(
    page_title="Bullet Inferno Roguelike v9",
    layout="centered",
...
<style>
    ...
    @media (max-width: 760px) {
        ...
        .shipGrid, .tipGrid, .relicGrid, .relicLibrary, .modeGrid { grid-template-columns: 1fr 1fr; }
...
function playerStatsHtml() {
    ...
    return `
        <div class="leaderboard">
            <div class="leaderTitle">Player Stats</div>
            ...
            <div class="smallNote">Most lethal boss: ${escapeHtml(topMapLabel(st.killedByBoss))} · Common death: ${escapeHtml(topMapLabel(st.deathTypes))} · Favorite ship: ${escapeHtml(topMapLabel(st.shipsUsed))} · Most played mode: ${escapeHtml(topMapLabel(st.modesPlayed))}</div>
        </div>
    `;
}
...
function modeButtonsHtml() {
    return Object.values(MODE_CONFIGS).map(m => `
...
function renderMenu() {
    ...
    showOverlay(`
        <div class="card">
            ...
            <div class="nameRow">
                ...
                <button class="primary" id="startBtn">Start Normal Run</button>
            </div>
            <div class="libraryHeader" style="margin-top:8px;">Game Modes</div>
            <div class="modeGrid">${modeButtonsHtml()}</div>
            ...
            <div class="libraryHeader">Relic Library</div>
            <div class="smallNote">These are the temporary relics that can appear between bosses during a run.</div>
            <div class="relicLibrary">${relicLibraryHtml()}</div>
            ${playerStatsHtml()}
            ${boardHtml()}
            <div class="rowButtons">
    ...
    document.querySelectorAll('.modeCard').forEach(btn => {
        btn.addEventListener('click', () => {
            playerName = (document.getElementById('nameInput').value || 'Pilot').trim().slice(0, 18) || 'Pilot';
            playSfx('start');
            startRun(btn.getAttribute('data-mode') || 'normal');
        });
    });
    ...
    if (clearStatsBtn) clearStatsBtn.addEventListener('click', () => {
        ...
        localStorage.removeItem(LAST_DEATH_KEY);
        playSfx('click');
        renderMenu();
    });
}
