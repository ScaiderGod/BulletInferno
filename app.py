...
st.set_page_config(
    page_title="Bullet Inferno Roguelike v9.5",
    layout="centered",
...
function getObjectCount(kind) {
    ...
    if (kind === 'rareItems') return bullets.filter(o => ['shield','time_fragment','magnet_pulse','risk_core','overcharge_cell','crit_core'].includes(o.type)).length;
...
function itemDropsHtml() {
    const items = [
        ...
        ['Shield','Rare','Grants 1 shield hit. Purpose: safe risky grabs. Combos with Broken Shield, Guardian Spark and Shrapnel Bloom.','0.35% base chance'],
        ['Power Up','Rare','Invulnerable for 3 seconds. Purpose: aggressive bomb collection window. Combos with Emerald Battery and Flash Guard.','1% base chance'],
        ['Critical Core','Rare','Your next bomb is guaranteed critical. Purpose: create burst damage. Combos with Critical Fuse, Godspark Fragment and Last Stand Fuse.','0.45% base chance'],
        ['Time Fragment','Mode','In Overheat, adds +5 seconds. Purpose: timer control. Combos with Coolant Heart, Time Broker and Chrono Drift.','0.65% in Overheat'],
        ['Magnet Pulse','Rare','Pulls nearby coins, bombs and hearts for 5 seconds. Purpose: safe collection route. Combos with Magnetic Orbit, Bomb Compass and Magnet Bank.','0.35% base chance'],
        ['Risk Core','Legendary','Big coin payout, but appears in dangerous patterns. Purpose: reroll/revive economy and risky build spikes. Combos with Volatile Risk Core.','0.12% base chance'],
        ['Overcharge Cell','Rare','For 6 seconds: faster ship and +25% bomb damage. Purpose: burst window. Combos with Overcharge Fuse and Solar Fuse.','0.25% base chance']
    ];
...
function currentBuildHint() {
    ...
    return `Current build tags: ${labels.slice(0,3).join(' · ')}. Reroll when the choices do not support these tags.`;
}
...
function projectileKind() {
    ...
    const pShield = 0.35;
    const pCritCore = 0.45 * luck * (1 + (mods.critCoreLuck || 0));
    const pMagnet = 0.35;
    const pOvercharge = 0.25;
    const pRisk = 0.12;
    const pTime = modeConfig.overheat ? 0.65 : 0;
    const pCoin = 8.5;
    const r = rand() * 100;
    let t = pGold;
    ...
    t += pInvuln;
    if (r < t) return 'power_invuln';
    t += pHeart;
    if (r < t) return 'heart';
    t += pShield;
    if (r < t) return 'shield';
    t += pCritCore;
    if (r < t) return 'crit_core';
    t += pTime;
    if (r < t) return 'time_fragment';
    t += pMagnet;
    if (r < t) return 'magnet_pulse';
    t += pOvercharge;
    if (r < t) return 'overcharge_cell';
    t += pRisk;
    if (r < t) return 'risk_core';
    t += pCoin;
    if (r < t) return 'coin';
    return 'bullet';
}
